...
async function scrapePrivacy() {
  const browser = await puppeteer.launch({ headless: true, args: ['--no-sandbox', '--disable-setuid-sandbox'] });
  try {
    const page = await browser.newPage();
    await page.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36');
    await page.goto('https://diravena.com/policies/privacy-policy', { waitUntil: 'networkidle2', timeout: 30000 });
    ...
    const data = { scrapedAt: new Date().toISOString(), source: 'https://diravena.com/policies/privacy-policy', type: 'privacy-policy', data: privacyData };
    const outputPath = path.join(__dirname, '..', 'data', 'diravena-privacy.json');
